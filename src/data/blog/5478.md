...
pubDatetime: 2015-05-21 07:03:00
description: "理解PATH环境变量在Linux中如何识别命令，如需在/usr/local/bin路径下运行命令而不指定完整路径，确保将/usr/local/bin加入用户PATH变异数列中。执行`echo $PATH`查看现有路径设置，或使用`export PATH=$PATH:/usr/local/bin`更新环境变量设置。"
tags: ["环境变量"]
---
...
![](https://linuxcn.img.undefined.today/data/attachment/album/201505/20/231450h329q23hy2ff9b9h.png)
