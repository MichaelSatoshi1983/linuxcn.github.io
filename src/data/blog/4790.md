...
pubDatetime: 2015-02-01 10:58:00
description: "查看Linux服务器运行级别：使用`who -r`、`runlevel`命令。确认默认网关：执行`route -n`、`netstat -nr`。重建初始化内存镜像：在CentOS 5.X / RHEL 5.X中，运用`mkinitrd`命令生成特定内核版本的初始化内存盘文件。"
tags: ["命令行", "面试"]
---

![](https://linuxcn.img.undefined.today/data/attachment/album/201502/01/110434rzyq6dn6z191rxiz.jpg)
