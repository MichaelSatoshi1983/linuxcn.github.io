...
pubDatetime: 2015-06-10 10:04:53
description: "在Ubuntu 13.04上遭遇404 Not Found错误阻止了使用apt-get或aptitude执行软件包安装或更新？这意味着仓库不再可访问。为解决此问题，需手动更新本地本地缓存中的仓库列表，确保覆盖记录了当前系统可访问的仓库来源。通过执行 `sudo rm -rf /var/lib/apt/lists/*`，清除过时的官方网站（Ubuntu.org或认证第三方存储库）列表文件。随后，使用 `sudo apt-get update` 重载仓库，系统将重新评估可访问的资源，极大地改善了软件包管理的可能性。请注意，建议在执行操作前备份重要数据。"
tags: ["Ubuntu", "更新"]
---
...
![](https://linuxcn.img.undefined.today/data/attachment/album/201506/10/100455jvbe66lhzzl2rgg8.jpg)
